...
router.get('/:issue_container/config.json.zip', async function (req, res) {
    ...
    try {
        ...
        // const zip = spawn(process.execPath, ['-P', process.env.CLI_CONFIG_PASSWORD, 'config.zip', 'config.json'], {
        //     cwd: tmpDir
        // }).on('error', (error) => {
        //     console.log(error);
        // });
...
router.get('/:issue_container/:issue/attachments/:id', async function (req, res) {
    ...
    try {
        ...
        if (match) {
            ...
            res.download(file_full_path_name).render('message', { session: req.session, message: `The file have been successfully downloaded to:  ${file_full_path_name}.` });
